        ...
        if z_max !=0 : 
            ...
            for id_sf, surface in enumerate(Surfaces):
                if surface.isPointInside( inter_pt , epsilon = 10e-3) :
                    ...
                    #print(idx)
                    #print(len(Surfaces))
                    #if abs(np.dot(surface.ineq[-1,:],inter_pt) - surface.ineq_vect[-1] )< 10e-3 :
                        #print("in the plan")
                        # inside the plan
                        #Sx = np.dot(surface.ineq[:-1,:], inter_pt)
                        #print( np.sum(Sx <= surface.ineq_vect[:-1]) == len(surface.vertices) )

            if id_surface is None : 
                # create new surface       
