...
class FootStepPlannerQP:
    ...
    def update_remaining_time(self ) : 
        ...
        if (self.k % self.k_mpc) == 0 :
            ...
            self.feet = np.where(self.gaitPlanner.current_gait[0, :] == 0)[0]
            ...
            for i in self.feet :             
                ...
                value =  self.t_swing[i] - (self.remainingTime * self.k_mpc - ((self.k+1)%self.k_mpc) )*self.dt_wbc - self.dt_wbc
    ...
    def computeNextFootstep(self , i, j , b_vlin , b_vref , feedback_term = True) :
        ...
        t_stance = self.gaitPlanner.getPhaseDuration(i, j, 1.0)  # 1.0 for stance phase
    ...
    def updateNewContact(self):
        ...
        for i in range(4) :    
            if self.gaitPlanner.current_gait[0,i] == 1.0 :
