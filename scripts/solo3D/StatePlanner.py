    ...
    def computeReferenceStates(self, q,  v, o_vref, z_average, new_step=False):
        ...
        if self.INSIDE_MAP:
            ...
            for k in range(self.n_steps):
                ...
                if k == 0:
                    self.surface_point = np.array([self.heightMap.x[i], self.heightMap.y[j], self.surface_equation[0]*self.heightMap.x[i] +
                                                   self.surface_equation[1]*self.heightMap.y[j] + self.surface_equation[2]])
