...
class StatePlanner():
    ...
    def computeReferenceStates(self, q,  v, o_vref, z_average, new_step=False):
        ...
        isInside_min, i_min, j_min = self.heightMap.find_nearest(q[0, 0:1] - self.FIT_SIZE_X, q[1, 0:1] - self.FIT_SIZE_Y)
        isInside_max, i_max, j_max = self.heightMap.find_nearest(q[0, 0:1] + self.FIT_SIZE_X, q[1, 0:1] + self.FIT_SIZE_Y)
        ...
        if isInside_min and isInside_max:
            ...
            rot_max = 0.3  # rad.s-1
            r_des = - 1.*np.arctan2(result[1], 1.)
            p_des = - 1.*np.arctan2(result[0], 1.)
            k_sp = 100
            epsilon = 0.01
            ...
            for k in range(self.n_steps):
                ...
                self.referenceStates[2, k + 1] = result[0]*self.heightMap.x[i] + result[1]*self.heightMap.y[j] + result[2] + self.h_ref

                if k == 0:
                    self.surface_point = np.array([self.heightMap.x[i], self.heightMap.y[j], result[0]*self.heightMap.x[i] + result[1]*self.heightMap.y[j] + result[2]])
