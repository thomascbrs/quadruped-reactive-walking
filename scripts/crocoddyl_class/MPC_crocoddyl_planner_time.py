...
class MPC_crocoddyl_planner_time():
    ...
    def __init__(self, params,  mu=1, warm_start=False, min_fz=0.0):
        ...
        self.gait_old = np.zeros(4)            # Last gait matrix
        ...
        self.stopWeights = np.zeros(8)           # Weights on the previous position predicted
    ...
    def update_model_augmented(self, model, terminal=False):
        ...
        if terminal:
            ...
            model.stateWeights = self.terminal_factor * model.stateWeights
    ...
    def solve(self, k, xref, footsteps, l_stop, velocity, acceleration):
        """
        ...
        Args:
            ...
            footsteps (Array 3x4): current position of the feet (given by planner)
        ...
        self.updateProblem(k, xref, footsteps, l_stop, velocity, acceleration)
    ...
    def updateProblem(self, k, xref, footsteps, l_stop, velocity, acceleration):
        ...
        while np.any(self.gait[j, :]):
            if j == 0:  # First row, need to take into account previous gait
                if np.any(self.gait[0, :] - self.gait_old):  # No optimisation of the time, end of current flying phase here
                    # Step model
                    ...
                    # Augmented model
                    self.models_augmented[index_augmented].updateModel(np.reshape(footsteps[j, :], (3, 4), order='F'), l_stop,
                    ...
                    # Step time model
                    self.models_time[index_time].updateModel(np.reshape(footsteps[j, :], (3, 4), order='F'),
                                                             xref[:, j+1], self.gait[j, :])
